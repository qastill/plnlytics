// vegetation.js - PLN Lytics Vegetation & ROW Layer v2
// Auto-loads when map page is shown

(function(){
        'use strict';

     // Wait for MAP to be available AND map container visible
     ...
     function initVegetation(){
            ...
            function createTreeIcon(color,size){
                            var s=size||20;
                            var svg='<svg viewBox="0 0 24 24" width="'+s+'" height="'+s+'">'
                            +'<polygon points="12,2 4,14 8,14 5,22 19,22 16,14 20,14" fill="'+color+'" opacity="0.9"/>'
                            +'<rect x="10" y="18" width="4" height="4" fill="#5a3" opacity="0.7"/>'
                            +'</svg>';
            ...
            var bbox='-6.50,107.95,-6.20,108.50';
                 var query='[out:json][timeout:90];(node["natural"="tree"]('+bbox+');way["natural"="wood"]('+bbox+');way["natural"="scrub"]('+bbox+');way["landuse"="forest"]('+bbox+');way["landuse"="orchard"]('+bbox+'););out center;';
            ...
            function processVegData(elements){
                            ...
                            map.eachLayer(function(l){
                                                if(l instanceof L.Polyline && !(l instanceof L.Polygon)){
                                                                        var ll=l.getLatLngs();
                                                                        if(ll.length>0 && ll[0] instanceof L.LatLng){
                                                                                                    for(var i=0;i<ll.length;i+=3){pCoords.push({lat:ll[i].lat,lng:ll[i].lng})}
                                                                        } else if(ll.length>0 && Array.isArray(ll[0])){
                                                                                                    ll.forEach(function(seg){for(var i=0;i<seg.length;i+=3){pCoords.push({lat:seg[i].lat,lng:seg[i].lng})}});
                                                                        }
                                                }
                            });

                     var sampled=pCoords.length>2000?pCoords.filter(function(_,i){return i%Math.ceil(pCoords.length/2000)===0}):pCoords;
                            var vegProcessed=[],riskCounts={KRITIS:0,TINGGI:0,SEDANG:0,RENDAH:0};
                            var typeLabels={tree:'Pohon',wood:'Hutan',scrub:'Semak',forest:'Hutan',orchard:'Kebun'};
                     ...
                     elements.forEach(function(e){
                                         ...
                                         +'</table>'
                                         +'<div style="margin-top:6px;padding:6px;background:'+(risk==='KRITIS'?'#fff0f0':risk==='TINGGI'?'#fff5e6':'#f0fff0')+';border-radius:4px;font-size:11px;">'
            ...
            function addLayerControls(counts){
                     ...
                     var vs=document.createElement('div');
                            ...
                            vs.innerHTML='<h4 style="color:#4ade80;margin:0 0 4px">\ud83c\udf33 VEGETASI & ROW <span style="font-size:9px;color:#8899aa;font-weight:400">('+
                                            (counts.KRITIS+counts.TINGGI+counts.SEDANG+counts.RENDAH)+' OSM)</span></h4>';
                     ...
                     [{k:'KRITIS',c:'#ff0000',l:'Kritis - Segera ROW'},{k:'TINGGI',c:'#ff6600',l:'Tinggi - Perlu ROW'},{k:'SEDANG',c:'#ffcc00',l:'Sedang - Monitor'},{k:'RENDAH',c:'#22cc44',l:'Rendah - Aman'}].forEach(function(r){
                                         ...
                                         sl.innerHTML='<input type="checkbox" checked class="veg-risk-filter" data-risk="'+r.k+'" disabled> <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:'+r.c+'"></span> '+r.l+' ('+counts[r.k]+')';
                                         vs.appendChild(sl);
                     });
                            ctrl.appendChild(vs);
                     ...
                     // Basemap configs
                     var bmCfg={
                                         dark:{url:'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',opt:{attribution:'CartoDB',maxZoom:19}},
                                         satellite:{url:'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',opt:{attribution:'Esri Satellite',maxZoom:19}},
            ...
            function addInfoPanel(vegData,counts,powerPts){
                     ...
                     var kritisVeg=vegData.filter(function(v){return v.risk==='KRITIS'});
                            var tinggiVeg=vegData.filter(function(v){return v.risk==='TINGGI'});
                            var loadPct=nStats.trafo>0?Math.round((totalMVA/(nStats.trafo*0.25))*100):0;
                            var vegDensity=totalVeg>0?Math.round(totalVeg/Math.max(nStats.segments,1)*100):0;
                     ...
                     h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><h3 style="margin:0;font-size:15px;color:#58a6ff">\u26a1 Analisis Jaringan & Vegetasi</h3>'
                            +'<span style="font-size:9px;color:#484f58">PLN Lytics Intelligence</span></div>';

                     h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:14px">';

                     // Card 1 - Jaringan
                     h+='<div class="analysis-card"><h4>\ud83d\udd0c Data Jaringan</h4>'
                            +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px">'
                            +'<div style="text-align:center;padding:6px;background:rgba(88,166,255,0.08);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#58a6ff">'+nStats.substations.length+'</div><div style="font-size:9px;color:#8b949e">Gardu Induk</div></div>'
                            +'<div style="text-align:center;padding:6px;background:rgba(88,166,255,0.08);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#58a6ff">'+nStats.feeders.size+'</div><div style="font-size:9px;color:#8b949e">Penyulang</div></div>'
                            +'<div style="text-align:center;padding:6px;background:rgba(88,166,255,0.08);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#58a6ff">'+nStats.segments.toLocaleString()+'</div><div style="font-size:9px;color:#8b949e">Segmen</div></div>'
                            +'<div style="text-align:center;padding:6px;background:rgba(88,166,255,0.08);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#58a6ff">'+nStats.trafo.toLocaleString()+'</div><div style="font-size:9px;color:#8b949e">Trafo</div></div>'
                            +'</div></div>';

                     // Card 2 - Beban
                     h+='<div class="analysis-card"><h4>\u26a1 Kondisi Beban</h4>'
                            +'<div style="text-align:center;padding:8px;background:rgba(88,166,255,0.08);border-radius:6px;margin-bottom:6px"><div style="font-size:22px;font-weight:700;color:'+(loadPct>85?'#ff4444':loadPct>70?'#ffcc00':'#22cc44')+'">'+totalMVA+' MVA</div><div style="font-size:9px;color:#8b949e">Daya Terpasang</div></div>'
                            +'<div style="font-size:10px;color:#8b949e;text-align:center">'+nStats.trafo.toLocaleString()+' Trafo | Est. Load '+(loadPct>100?'>100':loadPct)+'%</div>'
                            +'<div style="margin-top:6px;height:4px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden"><div style="height:100%;width:'+Math.min(loadPct,100)+'%;background:'+(loadPct>85?'#ff4444':loadPct>70?'#ffcc00':'#22cc44')+';border-radius:2px"></div></div>'
                            +'</div>';

                     // Card 3 - Vegetasi
                     h+='<div class="analysis-card"><h4>\ud83c\udf33 Vegetasi & ROW</h4>'
                            +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px">'
                            +'<div style="text-align:center;padding:6px;background:rgba(255,68,68,0.1);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#ff6b6b">'+counts.KRITIS+'</div><div style="font-size:9px;color:#8b949e">Kritis</div></div>'
                            +'<div style="text-align:center;padding:6px;background:rgba(255,153,0,0.1);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#ffaa33">'+counts.TINGGI+'</div><div style="font-size:9px;color:#8b949e">Tinggi</div></div>'
                            +'<div style="text-align:center;padding:6px;background:rgba(255,204,0,0.1);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#ffdd44">'+counts.SEDANG+'</div><div style="font-size:9px;color:#8b949e">Sedang</div></div>'
                            +'<div style="text-align:center;padding:6px;background:rgba(34,204,68,0.1);border-radius:6px"><div style="font-size:18px;font-weight:700;color:#44dd66">'+counts.RENDAH+'</div><div style="font-size:9px;color:#8b949e">Rendah</div></div>'
                            +'</div></div>';
                     ...
                     h+='<hr class="section-divider">';
                            h+='<h3 style="margin:0 0 10px;font-size:13px;color:#c9d1d9">\ud83d\udcca Analisis Detail & Rekomendasi</h3>';

                     // Network Analysis
                     h+='<div class="analysis-card"><h4>\ud83d\udd0c Kondisi Jaringan Distribusi</h4>';
                            h+='<p>Wilayah UP3 Indramayu memiliki <strong>'+nStats.substations.length+' Gardu Induk</strong> yang melayani <strong>'+nStats.feeders.size+' penyulang</strong> dengan total <strong>'+nStats.segments.toLocaleString()+' segmen</strong> jaringan tegangan menengah.</p>';
                            if(nStats.substations.length>0){
                                            h+='<p style="font-size:10px;color:#8b949e">GI: '+nStats.substations.join(' \u2022 ')+'</p>';
                            }
                            ...
                            h+='<p>Skor kesehatan jaringan: <span class="stat-badge '+(netScore>=4?'green':netScore>=3?'yellow':'red')+'">'+netScore+'/5.0</span></p>';
                            h+='</div>';

                     // Load Analysis
                     h+='<div class="analysis-card"><h4>\u26a1 Analisis Beban & Kapasitas</h4>';
                            h+='<p>Total daya terpasang <strong>'+totalMVA+' MVA</strong> dari <strong>'+nStats.trafo.toLocaleString()+' unit trafo distribusi</strong>. ';
                            if(loadPct>85){
                                            h+='Estimasi pembebanan <span class="stat-badge red">'+loadPct+'% (Overload)</span> menunjukkan kondisi kritis yang memerlukan penambahan kapasitas segera.</p>';
                            }else if(loadPct>70){
                                            h+='Estimasi pembebanan <span class="stat-badge orange">'+loadPct+'% (Tinggi)</span> mendekati batas optimal. Perlu perencanaan penambahan trafo.</p>';
                            }else{
                                            h+='Estimasi pembebanan <span class="stat-badge green">'+loadPct+'% (Normal)</span> masih dalam batas aman operasional.</p>';
                            }
                            h+='</div>';

                     // Vegetation Analysis
                     h+='<div class="analysis-card"><h4>\ud83c\udf33 Analisis Vegetasi & Hak Jalan (ROW)</h4>';
                            h+='<p>Teridentifikasi <strong>'+totalVeg+' titik vegetasi</strong> dari data OpenStreetMap di sepanjang koridor jaringan distribusi.</p>';
                            h+='<p>Distribusi risiko: '
                            +'<span class="stat-badge red">'+counts.KRITIS+' Kritis</span> '
                            +'<span class="stat-badge orange">'+counts.TINGGI+' Tinggi</span> '
                            +'<span class="stat-badge yellow">'+counts.SEDANG+' Sedang</span> '
                            +'<span class="stat-badge green">'+counts.RENDAH+' Rendah</span></p>';
                            if(counts.KRITIS>0){
                                            h+='<p>\u26a0\ufe0f <strong>'+counts.KRITIS+' vegetasi zona kritis</strong> (jarak <100m dari JTM) berpotensi menyebabkan gangguan flashover atau putus konduktor saat angin kencang.</p>';
                            }
                            if(counts.TINGGI>0){
                                            h+='<p>\ud83d\udfe0 <strong>'+counts.TINGGI+' vegetasi zona tinggi</strong> (100-300m) perlu dijadwalkan untuk pemangkasan preventif dalam 1-3 bulan ke depan.</p>';
                            }
                            h+='<p style="font-size:10px;color:#8b949e">\u26a0 Data vegetasi bersumber dari OSM. Perlu validasi survei lapangan untuk akurasi penuh.</p>';
                            h+='</div>';

                     // === SMART SUGGESTIONS ===
                     h+='<hr class="section-divider">';
                            h+='<h3 style="margin:0 0 10px;font-size:13px;color:#c9d1d9">\ud83d\udca1 Rekomendasi Tindakan</h3>';

                     if(counts.KRITIS>0){
                                     h+='<div class="suggestion-item critical"><div class="suggestion-icon">\ud83d\udea8</div><div class="suggestion-text"><strong>PRIORITAS 1 - Pemangkasan Darurat:</strong> Segera lakukan pemangkasan pada '+counts.KRITIS+' titik vegetasi kritis yang berjarak <100m dari saluran JTM. Risiko flashover dan gangguan padam tinggi terutama saat musim hujan/angin.</div></div>';
                     }
                            if(counts.TINGGI>0){
                                            h+='<div class="suggestion-item warning"><div class="suggestion-icon">\ud83d\udcc5</div><div class="suggestion-text"><strong>PRIORITAS 2 - Jadwalkan ROW:</strong> Rencanakan program Right of Way untuk '+counts.TINGGI+' titik vegetasi tingkat tinggi. Target penyelesaian 1-3 bulan. Koordinasi dengan Pemda untuk izin pemangkasan.</div></div>';
                            }
                            if(loadPct>80){
                                            h+='<div class="suggestion-item warning"><div class="suggestion-icon">\u26a1</div><div class="suggestion-text"><strong>Kapasitas Beban:</strong> Pembebanan trafo mencapai ~'+loadPct+'%. Pertimbangkan penambahan trafo sisipan atau uprating pada feeder dengan beban tertinggi untuk menghindari overload.</div></div>';
                            }
                            ...
                            h+='<div class="suggestion-item info"><div class="suggestion-icon">\ud83d\udcc8</div><div class="suggestion-text"><strong>Monitoring Berkala:</strong> Aktifkan layer satelit untuk inspeksi visual koridor jaringan. Pantau pertumbuhan vegetasi setiap 3 bulan menggunakan citra satelit terbaru.</div></div>';

                     if(nStats.switches<nStats.feeders.size){
                                     h+='<div class="suggestion-item info"><div class="suggestion-icon">\ud83d\udd27</div><div class="suggestion-text"><strong>Keandalan Jaringan:</strong> Rasio switching device ('+nStats.switches+') terhadap penyulang ('+nStats.feeders.size+') masih bisa ditingkatkan. Pertimbangkan pemasangan LBS/Recloser tambahan untuk mempercepat isolasi gangguan.</div></div>';
                     }
                            h+='<div class="suggestion-item info"><div class="suggestion-icon">\ud83c\udf0d</div><div class="suggestion-text"><strong>Tips:</strong> Gunakan basemap <strong>Satelit</strong> di panel Layers untuk melihat kondisi vegetasi secara visual. Klik marker pohon di peta untuk melihat detail dan rekomendasi per titik.</div></div>';
